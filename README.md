...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Parquet (beta)](https://www.cdata.com/download/download.aspx?sku=HIZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Parquet supported by the [CData JDBC Driver for Parquet](https://www.cdata.com/drivers/parquet/jdbc).
...
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/parquet-mcp-server-by-cdata.git
      cd parquet-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/parquet/download/jdbc](https://www.cdata.com/drivers/parquet/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Parquet\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Parquet/`
    * Run the command `java -jar cdata.jdbc.parquet.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.parquet.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `parquet.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=parquet
      ServerName=CDataParquet
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.parquet.jar
      DriverClass=cdata.jdbc.parquet.ParquetDriver
      JdbcUrl=jdbc:parquet:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
